...
import uuid
import razorpay
client = razorpay.Client(auth=("rzp_test_Szpk2T74rYYRdZ", "fhYCmzMCLl7RChcWKIFfizk5"))
#import razorpay
#client = razorpay.Client(auth=("rzp_test_Szpk2T74rYYRdZ", "fhYCmzMCLl7RChcWKIFfizk5"))
from werkzeug.utils import secure_filename #used to check secure filename or not
import os
mydb=connection.MySQLConnection(user='flaskuser',host='localhost',password='Diya@08',db='ecomdb')
app=Flask(__name__)
...
def adminlogin():
    ...
    try:
        ...
        login_password=data.get('password','').strip().encode('utf-8')
...
def additem():
    ...
    try:
        ...
        orig_secure=secure_filename(filename) #removes extra "/" or  spaces
        ext=os.path.splitext(orig_secure)[1]
        filename=genotp()+ext #'R7hW2k.ext'
        save_path=os.path.join(app.config['UPLOAD_FOLDER'],filename)#define image file path
        ...
        for item in allitems_data:
            products.append({'itemid':item[0],
                             ...
                             'price':float(item[4]),
                              'quantity':item[5],
                              'category':item[6],
                              'image':url_for('static',filename=f'uploads/{item[7]}',
                                              _external=True)})
...
@app.route('/api/admin/items/<itemid>', methods=['GET'])
def viewitem(itemid):
    cursor = None
    try:
        if 'adminid' not in session:
    ...
    try:
        ...
        if updateditem_filedata:
            ...
            if not updateditem_filedata.mimetype.startswith('image/'):
                return jsonify({'status':'failed','message':'Invalid image'}),500
            ...
            filename=genotp()+ext
            #save new image in static folder
        ...
        #update database
        cursor.execute('''update items set item_name=%s,item_description=%s,item_about=%s,quantity=%s,price=%s,category=%s,
                       item_filename=%s where itemid=uuid_to_bin(%s) and added_by=uuid_to_bin(%s)''',[updateditem_name,
                    updateditem_description,updateditem_about,updateditem_quantity,updateditem_price,updateditem_category,filename,itemid,adminid])
        mydb.commit()
        cursor.close()
...
def viewcart():
    ...
    try:
        ...
        for item in cart_items:
            ...
            items_data.append({'itemid':itemid,'itemname':itemname,'price':item_price,
            'quantity':item_quantity,'category':item_category,'image':image_url,'total':total})
        delivery=40
...
def pay_cart():
    ...
    try:
        ...
        for item in cart_items:
            ...
            image_url=url_for('static',filename=f'uploads/{item_imgname}',_external=True)
            items_data.append({'itemid':itemid,'itemname':itemname,'price':item_price,
            'quantity':item_quantity,'category':item_category,'image':image_url,'amount':amount})
        delivery=40
        tax=round(subtotal*0.05,2)
        grand_total=subtotal+tax+delivery
        ...
        return jsonify({'status':'success','order':{
            ...
            'razorpay_key':'rzp_test_Szpk2T74rYYRdZ'
...
def usersearch():
    ...
    try:
        ...
        if not pattern.match(searchdata):
            return jsonify({'status':'failed','meassage':'INVALID SEARCH'}),400
        #reconnect automatically if mysql connection lost
        mydb.ping(reconnect=True)
        cursor=mydb.cursor(buffered=True)
        cursor.execute('''select bin_to_uuid(itemid),item_name,item_description,item_about,price,quantity,category,
                       item_filename from items where item_name like %s or item_description like %s 
                       or price like %s or category like %s''',
...
def buy_now():
    ...
    try:
        ...
        return jsonify({'status':'success','message':'Single buy item stored successfully','payment_type':'single',
        'item':{
                'itemid':item_data[0],
                'itemname':item_data[1],
...
def get_reviews(itemid):
    ...
    try:
        ...
        # Check item exists
        cursor.execute(
            'SELECT COUNT(*) AS count FROM items WHERE itemid = uuid_to_bin(%s)',
            [itemid]
        )
        item_exists = cursor.fetchone()['count']
        ...
        cursor.execute("""
            SELECT
                bin_to_uuid(r.r_id) AS review_id,
                r.r_text,
                r.rating,
                u.username,
